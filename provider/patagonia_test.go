...
func TestPatagonia(t *testing.T) {
	...
	assert.Equal(t, "", hotels[0].City)
	assert.Equal(t, "", hotels[0].Country)
	...
	assert.Equal(t, "", hotels[1].Address)
	assert.Equal(t, "", hotels[1].City)
	assert.Equal(t, "", hotels[1].Country)
	assert.Equal(t, "", hotels[1].Description)
	...
	assert.Equal(t, "amenities", hotels[1].Images[2].Description)
