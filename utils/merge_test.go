...
func TestGetUniqueStrings(t *testing.T) {
	t.Run("no string", func(t *testing.T) {
		strs := []string{}
		assert.Equal(t, []string{}, utils.RemoveSimilarAndSubstrings(strs))
	})

	t.Run("1 string", func(t *testing.T) {
		strs := []string{"pool"}
		assert.Equal(t, []string{"pool"}, utils.RemoveSimilarAndSubstrings(strs))
	})

	t.Run("2 strings", func(t *testing.T) {
		strs := []string{"pool", "wifi"}
		assert.Equal(t, []string{"pool", "wifi"}, utils.RemoveSimilarAndSubstrings(strs))
	})
	...
	t.Run("iron and aircon", func(t *testing.T) {
		...
		assert.Equal(t, []string{"iron", "aircon"}, utils.RemoveSimilarAndSubstrings(strs))
	})

	t.Run("strings with substring", func(t *testing.T) {
		strs := []string{"pool", "wifi", "indoor pool"}
		assert.Equal(t, []string{"indoor pool", "wifi"}, utils.RemoveSimilarAndSubstrings(strs))
	})
}
